...
The summary statistics approach to group analysis. A first-level GLM is fit separately to each subject and contrast images ($c^T\hat{\beta}$, where $c$ is a contrast vector and $\hat{\beta}$ the estimated first-level coefficients) are created. At the second level, the per-subject contrast values are the data for a group model — e.g., a one-sample t-test on whether the contrast differs from 0 — yielding a group statistic map. *(Figure 21.1 from the book. © the authors and MIT Press; reproduced with permission — not covered by this site's CC-BY license.)*
:::
...
Formally, the first-level GLM for subject $i$, where $i$ indexes the $m$ subjects in the study, is
...
:::{div}
:class: eq-tip-text
yᵢ — subject i's fMRI time series (time × 1) · Xᵢ — subject i's design matrix (time × predictors) · βᵢ — subject-specific regression parameters · εᵢ — within-subject measurement noise · m — number of subjects
:::
::::
:::{div}
:class: eq-where
*where* $y_i$ *is the fMRI time-series vector for subject* $i$*,* $X_i$ *that subject's design matrix (time points × predictors),* $\beta_i$ *the subject-specific regression parameters,* $\epsilon_i$ *within-subject measurement noise, and* $m$ *the number of subjects.*
:::
...
:::{div}
:class: eq-tip-text
βᵢ — subject-specific parameters · Z_G — second-level (group) design matrix · β_G — population effects · ηᵢ — subject i's deviation from the group values · U_G — covariance of the random effects
:::
::::
:::{div}
:class: eq-where
*where* $Z_G$ *is the second-level design matrix (e.g., intercept plus group or covariate columns),* $\beta_G$ *the vector of population effects,* $\eta_i$ *subject* $i$*'s random deviation from those group values, and* $U_G$ *the covariance matrix of the random effects — the variances (and covariances) of true individual differences.*
:::

Substituting the second level into the first gives a single-level form whose composite error contains *both* variance components: the within-subject covariance (from $\epsilon_i$) plus the between-subject covariance (from $\eta_i$). A correctly specified **mixed effects model** estimates both components — typically by restricted maximum likelihood (ReML) with iterative algorithms such as EM — and weights each subject's contribution in proportion to its precision (inverse variance), so noisier subjects influence the group estimate less. The summary statistics approach implicitly treats subject as a random effect: because each subject contributes one number per voxel, variation across those numbers *is* the between-subject error term. When first-level standard errors are equal across subjects, the simple two-stage approach is valid and fully efficient — as powerful as the full mixed model.

In practice the summary statistics approach dominates fMRI (nearly 90% of papers): it is valid, computationally cheap, and combines naturally with robust regression, bootstrapping, and permutation tests. Simulation studies have found it surprisingly robust, with appropriate false positive control and near-optimal power under typical designs. Full mixed models are slower and easier to misuse, but they are more powerful for complex designs — longitudinal and twin studies, missing data, and variable data quality across people. Software occupies a spectrum of compromises, many using *precision-weighted summary statistics*: FSL's FLAME, AFNI's 3dMEMA, [SPM](https://www.fil.ion.ucl.ac.uk/spm/)'s spm_mfx, and CANlab's `igls.m` / `glmfit_multilevel.m` among others.
...
Outliers and robust regression. (A) In null-hypothesis data ($N = 50$, left), adding a single extreme point (center) flips the estimated slope and makes the correlation spuriously significant. The robust IRLS solution (right) automatically down-weights the outlier (open circle; lighter shading = lower weight). (B) Compared with OLS, IRLS yields stronger group activation in visual cortex for a visual task and contralateral somatosensory regions for a pain task. *(Figure 21.4 from the book. © the authors and MIT Press; reproduced with permission — not covered by this site's CC-BY license.)*
:::
...
n_subj = 20; n_trials = 40;    % n_subj = subjects, n_trials = trials per subject
sigma_between = 0.5;           % SD of true subject effects (real individual differences)
sigma_within  = 1.0;           % SD of trial-level (within-subject) noise

n_iter = 1000;                 % number of simulated experiments; more -> stabler FPR estimate
p_ffx = zeros(n_iter, 1); p_rfx = zeros(n_iter, 1);   % p-values per experiment
for it = 1:n_iter
...
n_subj, n_trials = 20, 40                # n_subj = subjects, n_trials = trials per subject
sigma_between, sigma_within = 0.5, 1.0   # between-subject SD, trial-level noise SD

n_iter, p_ffx, p_rfx = 1000, [], []      # n_iter = simulated experiments; more -> stabler FPR
...
```text
False positive rate at alpha = .05:  FFX = 0.514   RFX = 0.045
```
...
n_subj = 30;                                % subjects contributing one COPE each
perf = randn(n_subj, 1);                    % behavioral covariate (mean-centered)
cope = 0.3 .* randn(n_subj, 1);             % contrast values; true slope = 0
perf(end) = 4; cope(end) = 3;               % make the last subject an extreme outlier
...
n_subj = 30                                 # subjects contributing one COPE each
perf = rng.standard_normal(n_subj)          # behavioral covariate (mean-centered)
cope = 0.3 * rng.standard_normal(n_subj)    # contrast values; true slope = 0
perf[-1], cope[-1] = 4, 3                   # make the last subject an extreme outlier
...
```text
Slope:  OLS = 0.281   robust = -0.036
Robust weight for outlier subject: 0.000
```
