...
The linear system framework used in fMRI. An experimental stimulus function — a block design (left) or event-related design (right) — is convolved with a canonical HRF to obtain the predicted BOLD response. *(Figure 18.2 from the book.)*
:::
...
where $y$ is the $n \times 1$ voxel time series, $X$ is $n \times p$, $\beta$ is a $p \times 1$ vector of unknown amplitudes, and $\epsilon$ collects unexplained error.

:::{figure} images/ch18_fig3_glm_setup.png
:alt: fMRI data equals design matrix times betas plus residuals
:width: 85%

A pictorial representation of the standard GLM setup. The fMRI time series from one voxel (left) is modeled as the design matrix — here an intercept plus two task regressors — multiplied by three beta values, plus residuals. *(Figure 18.3 from the book.)*
:::
...
Creating a design matrix with four conditions. Indicator functions for the timing of conditions A–D are convolved with a canonical HRF to obtain four predicted responses, which become columns of the design matrix. *(Figure 18.4 from the book.)*
:::
...
TR = 2; n_scans = 180;                        % 180 volumes, 6 min run
...
t_r, n_scans = 2.0, 180                        # 180 volumes, 6 min run
...
**Step 2 — Fit the GLM to a voxel time series.** We simulate a voxel with known true effects ($\beta_A = 0.8$, $\beta_B = 0.4$, intercept 100) and recover them with the OLS formula $\hat{\beta} = (X^TX)^{-1}X^Ty$.
...
rng(42);
beta_true = [0.8 0.4 100]';                    % A, B, intercept (last col)
y = X * beta_true + 2 * randn(n_scans, 1);
...
```python
# Simulate a voxel: y = X @ beta_true + noise
Xm = X.to_numpy()                              # columns: A, B, constant
rng = np.random.default_rng(42)
beta_true = np.array([0.8, 0.4, 100.0])
y = Xm @ beta_true + 2 * rng.standard_normal(n_scans)
...
The estimates should land close to the true values, with strong t-statistics for both conditions. The full labs continue the arc: inspecting residuals and model fit ($R^2$), computing standard errors and P values, and testing a simple **A − B contrast** — the bridge to Chapter 20.
