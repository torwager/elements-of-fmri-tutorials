...
which follows an approximate t-distribution under $H_0$. For an F-contrast, the full model is compared with a **reduced model** whose design matrix omits the subspace spanned by the contrasts — the F-statistic asks how much *extra* variance the effects of interest explain:

$$
F = \frac{(r_0^T r_0 - r^T r)\,/\,\nu_1}{r^T r\,/\,\nu_2}
$$

where $r_0$ and $r$ are the reduced- and full-model residuals, $\nu_1$ is the number of independent effects tested (the model degrees of freedom of the contrast), and $\nu_2$ is the error degrees of freedom. In the massively univariate approach, these statistics are computed at every voxel, producing the t- and F-maps that thresholding and multiple-comparisons procedures (Chapter 22) then operate on.
...
y = X * beta_true + 0.15 * randn(n_scans, 1);
...
y = Xm @ beta_true + 0.15 * rng.standard_normal(n_scans)
